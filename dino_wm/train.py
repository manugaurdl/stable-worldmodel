...
class Trainer:
    def __init__(self, cfg):
        ...
        self.epoch = 0
        self.global_step = 0
        # log per-step train scalars to wandb every N train iters (prejepa-style);
        # configurable via training.log_every_n_steps, default 50.
        self.log_freq = self.cfg.training.get("log_every_n_steps", 50)
        ...
        if self.accelerator.is_main_process:
            ...
            if self.cfg.debug:
                ...
                self.wandb_run = wandb.init(
                    project="stable-wm",   # repro patch D7: log to manugaur/stable-wm
                    entity="manugaur",
                    config=wandb_dict,
                    id=wandb_run_id,
                    resume="allow",
            ...
            # train scalars are logged per-step on wandb's default step axis; the
            # per-epoch metrics below are pinned to an `epoch` x-axis so they don't
            # scatter across the high-frequency train steps (prejepa: train on_step,
            # val on_epoch).
    ...
    def train(self):
        ...
        for i, data in enumerate(
            ...
            # True pre-clip grad norm, logged as train/grad_norm (mirrors prejepa).
            # Under AMP the grads are still loss-scaled here, so divide by the
            # GradScaler scale to recover the real norm; max_norm=inf only measures
            # the norm, it does not alter the grads.
            ...
            # ---- per-step train logging (every log_freq iters), prejepa-style ----
            if (
                self.accelerator.is_main_process
                and self.global_step % self.log_freq == 0
            ):
                step_dt = time.perf_counter() - step_t0
                global_bs = obs["visual"].shape[0] * self.accelerator.num_processes
                lr = (
                    self.predictor_optimizer.param_groups[0]["lr"]
                    if self.cfg.has_predictor
                    else self.encoder_optimizer.param_groups[0]["lr"]
                )
                step_log = {
                    "train/loss": loss.item(),
                    "train/grad_norm": grad_norm.item(),
                    "train/lr": lr,
                    "perf/samples_per_sec": global_bs / max(step_dt, 1e-8),
                    "perf/step_time_s": step_dt,
                }
                step_log.update({f"train/{k}": v for k, v in loss_components.items()})
            ...
            if self.cfg.has_decoder and plot:
                ...
                if self.cfg.has_predictor:
                    ...
                    err_logs = {f"train/{k}": [v] for k, v in err_logs.items()}
                ...
                if visual_out is not None:
                    for t in range(
                        ...
                        img_pred_scores = {
                            f"train/img_{k}_pred": [v.mean().item()]
                ...
                if visual_reconstructed is not None:
                    for t in range(obs["visual"].shape[1]):
                        ...
                        img_reconstruction_scores = {
                            f"train/img_{k}_reconstructed": [v.mean().item()]
        ...
        for i, data in enumerate(
            ...
            if self.cfg.has_decoder and plot:
                ...
                if self.cfg.has_predictor:
                    ...
                    err_logs = {f"val/{k}": [v] for k, v in err_logs.items()}
                ...
                if visual_out is not None:
                    for t in range(
                        ...
                        img_pred_scores = {
                            f"val/img_{k}_pred": [v.mean().item()]
                            for k, v in img_pred_scores.items()
                        }
                        self.logs_update(img_pred_scores)
                ...
                if visual_reconstructed is not None:
                    for t in range(obs["visual"].shape[1]):
                        ...
                        img_reconstruction_scores = {
                            f"val/img_{k}_reconstructed": [v.mean().item()]
                            for k, v in img_reconstruction_scores.items()
                        }
                        self.logs_update(img_reconstruction_scores)
            ...
            loss_components = {f"val/{k}": [v] for k, v in loss_components.items()}
