...
class VWorldModel(nn.Module):
    ...
    def encode_obs(self, obs):
        ...
        visual = obs['visual']
        b = visual.shape[0]
        # repro patch D10 (efficiency only; frozen encoder => identical features): if
        # 'visual' is already precomputed patch features (b,t,num_patches,emb_dim) -> ndim 4,
        # use them directly and skip encoder_transform + DINOv2. Live images are ndim 5
        # (b,t,3,H,W) and still go through the encoder (e.g. during planning eval).
        if visual.ndim == 4:
            visual_embs = visual
        else:
