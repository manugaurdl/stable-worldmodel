...
class TrajSlicerDataset(TrajDataset):
    ...
    def __getitem__(self, idx):
        ...
        # repro patch D9 (efficiency only; output bit-identical to original): decode ONLY
        # the num_frames visual frames this slice needs, instead of the whole episode video.
        # Original did `self.dataset[i]` which decodes all T frames per slice -> O(T)
        # redundant decode (~30x slower; full epoch ~16h). Actions still span all frames in
        # [start,end) and are read straight from the (already-normalized) action tensor.
        frame_idx = list(range(int(start), int(end), self.frameskip))  # num_frames indices
        obs, _act_sub, state, _ = self.dataset.get_frames(i, frame_idx)
