...
- 18,685 trajectories
- 2,336,736 total steps (~125 steps/episode; range 49–246)

**Each timestep in trajectory stores:**

- `pixels` — the rendered frame (PNG bytes, decoded to 224×224 image)
- `state` (7D) — full env state: agent x,y · block x,y · block angle · agent vx,vy
- `proprio` (4D) — agent only: x,y,vx,vy
- `action` (2D) — the expert's commanded move at that step

**A trajectory** = one full expert push episode: the ordered sequence of these steps from start to goal.



### coverage limited to expert dataset; no counterfactuals (CEM samples actions randomly; dataset not from policy)
